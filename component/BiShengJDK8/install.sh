...
if [[ ${verify_signature} -eq "0" ]]; then
    if [[ ! -d ${HOME}/compilers/bisheng-jdk1.8.0_402 ]]; then
        mkdir -p ${HOME}/compilers

        echo "Decompress bisheng-jdk-8u402-linux-aarch64.tar.gz to ${HOME}/compilers."
        tar -zxf /tmp/devkitdependencies/bisheng-jdk-8u402-linux-aarch64.tar.gz -C ${HOME}/compilers
        echo "Decompress bisheng-jdk-8u402-linux-aarch64.tar.gz to ${HOME}/compilers finished."
    fi
    ...
    if [[ ${java_path} != ${HOME}/compilers/bisheng-jdk1.8.0_402/bin/java ]]; then
        ...
        for BinFilePath in ${HOME}/compilers/bisheng-jdk1.8.0_402/bin/*; do
