...
if [[ ${verify_signature} -eq "0" ]]; then
    if [[ ! -d ${HOME}/compilers/bisheng-jdk-17.0.10 ]]; then
        mkdir -p ${HOME}/compilers

        echo "Decompress bisheng-jdk-17.0.10-linux-aarch64.tar.gz to ${HOME}/compilers."
        tar -zxf /tmp/devkitdependencies/bisheng-jdk-17.0.10-linux-aarch64.tar.gz -C ${HOME}/compilers
        echo "Decompress bisheng-jdk-17.0.10-linux-aarch64.tar.gz to ${HOME}/compilers finished."
    fi
    ...
    if [[ ${java_path} != ${HOME}/compilers/bisheng-jdk-17.0.10/bin/java ]]; then
        ...
        for BinFilePath in ${HOME}/compilers/bisheng-jdk-17.0.10/bin/*; do
