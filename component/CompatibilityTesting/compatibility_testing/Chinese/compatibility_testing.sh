...
check_configuration() {
    ...
    if [[ -f "${config_file}" ]]; then
        ...
        start_performance_time=$(sed '/^start_performance_time=/!d;s/start_performance_time=//' "${config_file}")
        ...
        idle_performance_time=$(sed '/^idle_performance_time=/!d;s/idle_performance_time=//' "${config_file}")
        ...
        if [[ -n "${start_performance_scripts}" && ! "${start_performance_time}" -gt 0 ]]; then
            write_messages e 31 2 "压力测试工具运行时间必须大于0"
            exit 1
        else
            TEST_TOOL_TIME=${idle}
        fi
        if [[ ! "${idle_performance_time}" -gt 0  ]]; then
            write_messages e 31 2 "空载采集时间需提供或空载采集时间必须大于0."
	    exit 1
        else
            IDLE_PERFORMNCE_TIME=${idle_performance_time}
        fi
...
# 4.2、idle_0采集
get_performance 5 ${IDLE_PERFORMNCE_TIME} 0 4
...
if [[ ${HPC_CERTIFICATE} -eq 0 && ${STRESS_CMD} -eq 1 ]]; then
    ...
    # 7.2、压力测试采集
    sleep 2
    get_performance 5 ${TEST_TOOL_TIME} 1 7
    sleep 20
fi
